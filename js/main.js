...
document.addEventListener("DOMContentLoaded", (event) => {
    const highlight = document.getElementById("highlight-style"); //set this id highlight-style to style object

    gsap.registerPlugin(ScrollTrigger);

    gsap.utils.toArray(".text-highlight").forEach((highlight) => { //add the highlight element css property to the element
        ScrollTrigger.create({
            trigger: highlight,
            start: "-100px center",
            onEnter: () => highlight.classList.add("active") //start highlighting immediately
        });
    });

    //coin scroll trigger
    gsap.utils.toArray(".attribute").forEach(function(elem, i) { //for each attribute  element we need to add the attribute
    ...
    document.body.setAttribute("data-highlight", value); //set the highlight style to style object from the document body attribute value

    highlight.addEventListener("change", (e) => setHighlightStyle(e.target.value)); //set highlight style to value attribute on change
...
document.addEventListener("DOMContentLoaded", (event) => {
    const scroller = new LocomotiveScroll({
        el: document.querySelector('[data-scroll-container]'), //use locomotive scroll
        smooth: true
    });
});
